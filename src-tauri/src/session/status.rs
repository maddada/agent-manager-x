...
/// - If message is stale (>30s old) and file not recently modified -> Waiting/Idle (session is inactive)
...
pub fn determine_status(
    ...
    // Early return: If message is stale and file not recently modified,
    // the session is NOT actively processing
    if message_is_stale && !file_recently_modified {
        return match last_msg_type {
            Some("assistant") => SessionStatus::Waiting,
            Some("user") => SessionStatus::Waiting,
